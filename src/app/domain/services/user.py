...
from src.app.domain.schemas.user.requests.post import AddUserSchemaRequest, AddUserSchemaRequestWithHashedPassword
...
class UserService(Service, UserServiceInterface):
    ...
    def get(self, schema: GetUserSchemaRequest) -> dict[str: list] | HTTPException:
        get_params = {}
    ...
    def add(self, schema: AddUserSchemaRequest) -> dict[int: Optional[int], str: bool] | HTTPException:
        ...
        new_id = self.repository.add(
            AddUserSchemaRequestWithHashedPassword(**params | {"hashed_password": hashed_password})
        )
